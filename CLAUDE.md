...
3. **State is per-worker, read-only after bind (by convention + process isolation):** `State` in `sales_logic/globals.py` is a module-global class populated once per worker process by `bind_globals()`. There is **no runtime immutability enforcement** — the old `_SealableMeta` metaclass + `seal()` were removed (they were never called in production, and could not be, since a lazy per-worker cache lived on `State`). Immutability of the bound dimension/config fields rests on (a) convention (bind once, never reassign) and (b) process isolation: each worker has its own `State`. The one field reassigned after bind is the lazy `State._sales_channels_cache` (first-use, per worker). Note: customer *discovery* is a static `State.customer_discovery_month` schedule (computed once in the coordinator, broadcast read-only) so the sales fact is worker-count invariant; do not reintroduce cross-chunk mutable discovery state. Treat the bound fields as read-only after `bind_globals`. In tests, call `State.reset()` between cases. The determinism guarantee is enforced by the Phase-0 guardrail tests (worker- and chunk-invariance), not by a runtime seal. (The old `SalesContext` dependency-injection dataclass was removed — it was dead.)
