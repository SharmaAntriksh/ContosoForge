...
def compose_postgres_constraints_sql(
    *, sql_root: Path, sales_cfg: dict, cfg: dict | None = None
    ...
    parts: list[Path] = [pg_dir / "00_dimensions.sql"]
    if mode in {"sales", "both"}:
        parts.append(pg_dir / "10_sales.sql")
    if mode in {"sales_order", "both"}:
        parts.extend(
            [
                pg_dir / "20_sales_order_header.sql",
                pg_dir / "21_sales_order_detail.sql",
                pg_dir / "22_sales_order_relations.sql",
            ]
        )
    budget_constraints = pg_dir / "30_budget.sql"
    if budget_constraints.exists() and _budget_enabled(cfg):
        parts.append(budget_constraints)
    inventory_constraints = pg_dir / "40_inventory.sql"
    if inventory_constraints.exists() and _inventory_enabled(cfg):
        parts.append(inventory_constraints)
    ...
    chunks: list[str] = []
    chunks.append("-- Auto-generated by packaging: composed Postgres constraints\n")
    chunks.append(f"-- mode: {mode}\n")
    for p in existing:
        chunks.append(
            "\n-- ============================================================\n"
